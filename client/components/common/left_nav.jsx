let { Paper, Overlay, Styles, Mixins, Utils } = mui;
...
LeftNav = React.createClass({
  ...
  getThemePalette() {
    return this.context.muiTheme.palette || this.context.muiTheme.rawTheme;
  },

  getTheme() {
    return this.context.muiTheme.leftNav
      || this.context.muiTheme.component.leftNav;
  },

  getStyles() {
    console.log(document.body.clientWidth);
    let x = this._getTranslateMultiplier() *
      (this.props.isOpen ? 0 : this._getMaxTranslateX());
    ...
    let styles = {
      root: {
        ...
        position: (this.props.from==="property_nav"&&document.body.clientWidth<800? 'static':'fixed'),
        zIndex: 10,
        left: 0,
        top: 0,
      ...
      menuItem: {
        height: spacing.desktopLeftNavMenuItemHeight,
        lineHeight: spacing.desktopLeftNavMenuItemHeight + 'px',
      },
      rootWhenOpenRight: {
        left: 'auto',
        right: 0,
  ...
  render() {
    ...
    if (!this.props.docked) {
      overlay = (
        <Overlay
          ref="overlay"
          show={this.props.isOpen || !!this.state.swiping}
  ...
  _setPosition(translateX) {
    ...
    leftNav.style[AutoPrefix.single('transform')] =
      'translate3d(' +
        (this._getTranslateMultiplier() * translateX) +
        'px, 0, 0)';
  ...
  _getTranslateX(currentX) {
    return Math.min(
      Math.max(
        ...
        this._getMaxTranslateX() - this._getTranslateMultiplier() *
  ...
  _onBodyTouchMove(e) {
    ...
    if (this.state.swiping) {
      ...
      if (dXAbs > threshold && dYAbs <= threshold) {
        this._swipeStartX = currentX;
        this.setState({
          swiping: this.props.isOpen ? 'closing' : 'opening',
        });
        this._setPosition(this._getTranslateX(currentX));
  ...
  _onBodyTouchEnd(e) {
    if (this.state.swiping) {
      ...
      let translateRatio =
        this._getTranslateX(currentX) / this._getMaxTranslateX();
